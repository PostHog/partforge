...
Compaction normalizes artifacts that contain multiple physical parts in one destination partition, then batches single-part artifacts from the same destination partition into one part. It lets ClickHouse select background merges.
...
graph TD
    ...
    J --> L[Wait for destination merges]
...
After a successful insert-select, the worker applies these destination table settings with `ALTER TABLE ... MODIFY SETTING` and proceeds to merge waiting without restarting ClickHouse:
...
When `worker -compact=true` finds no `READY` source part, it waits for a small derived random splay and then looks for a `COMPACT_READY` artifact containing multiple physical parts in one destination partition. It preserves the partition-collision rules, then claims the eligible artifact with the most persisted physical parts and normalizes it.

Rewrites usually produce much smaller output than their source, so one output per source part would leave the destination cluster with many small parts to merge. When no fragmented artifact is available, the worker batches single-part artifacts (`compact_normalized`) from one job and destination partition. In a single transaction it locks the smallest such artifact that has a sibling fitting `-compact-max-bytes`, then locks up to `-compact-max-artifacts` of the smallest unlocked siblings with `FOR UPDATE SKIP LOCKED`, and trims them to the byte cap. Concurrent claimers therefore never share a row; a claimer left with fewer than two artifacts claims nothing. The batch attaches every input and merges each partition to one part through the same normalization path as a fragmented artifact. The output is itself a single-part artifact and can batch again with later siblings until the byte cap stops it.

A single-part artifact is promoted to `FINISHED` once nothing can still merge with it: no sibling in its partition fits the byte cap, no compacting batch or fragmented artifact touches its partition, and the job has no `READY` or `IN_PROGRESS` rewrite that may produce one. The compact window still finalizes everything when it expires. Artifacts with one part in each of several partitions are not batched. `-compact-max-artifacts=1` disables batching and restores immediate finalization of single-part artifacts.
