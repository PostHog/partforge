...
Workers run insert-select queries with the detected CPU count and an insert-heavy memory cap. Before the insert starts, the worker applies `default_compression_codec` to the worker destination table; the default is `ZSTD(5)` and can be changed with `-default-compression-codec`. After the insert completes, the worker applies memory-derived `merge_max_block_size` and `merge_max_block_size_bytes`, a lower local `merge_selecting_sleep_ms`, and output-size-derived `max_bytes_to_merge_at_max_space_in_pool` and `max_bytes_to_merge_at_min_space_in_pool` to the worker destination table. The max-space merge cap targets about four large output parts up to 150 GiB per automatic merge, while the min-space cap stays between 1 GiB and 32 GiB so small and medium parts can still compact when merge disk reservations are tight. The worker then restarts its local ClickHouse child process with `background_pool_size` set to the larger of the detected CPU count and ClickHouse's minimum valid pool size for default MergeTree mutation settings and `background_merges_mutations_scheduling_policy = 'shortest_task_first'`. It then lets destination merges run. Merge debt, output size, and the idle `OPTIMIZE FINAL` threshold are measured from `system.parts.bytes_on_disk`. If no merge is active but more than one active output part remains, the worker keeps polling until merges have stayed idle and the active output part count has stayed unchanged for at least two minutes before treating merges as settled. If that idle state still has small-part debt and the active output is no larger than `-merge-idle-optimize-final-max-bytes` on disk, the worker runs one local `OPTIMIZE TABLE ... FINAL` before freezing. After the base `-merge-timeout`, the worker only continues while small output part debt remains and useful merge progress is still visible, and `-merge-hard-timeout` is the absolute ceiling. Use `upload-freeze -optimize-final` to record that this job's workers should run `OPTIMIZE TABLE ... FINAL` on their local worker destination table after each insert-select. Use `worker -optimize-final` to force that same local worker optimize for every job processed by that worker. When explicit optimize final is enabled, the worker runs local `OPTIMIZE TABLE ... FINAL` with `optimize_throw_if_noop=1`, `send_timeout=0`, and `receive_timeout=0`, waits if `system.merges` shows that ClickHouse is still merging after the client request returns, and checks `system.part_log` for failed destination merges before retrying. It repeats with 1-minute, 2-minute, 3-minute retry delays up to 10 minutes until `system.parts` reports at most one active destination part per partition, failing with the recorded ClickHouse merge error if ClickHouse reports one. PartForge never runs `OPTIMIZE FINAL` on the source/upload host.
