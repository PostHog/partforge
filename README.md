...
For the detailed worker procedure, see [docs/rewrite-flow.md](docs/rewrite-flow.md).
...
The worker image is a single Ubuntu-based container with ClickHouse packages, `s5cmd`, and the Go binary copied in from a builder stage. Its entrypoint is the Go worker binary, and the worker runs as root so it can create and write the resolved worker work directory on root-owned host mounts. The worker starts `clickhouse server` as a child process for each claimed part from DynamoDB. The default ClickHouse version is `26.3.10.60`.

Use `partforge worker -role=inserter` for the insert-select stage and `partforge worker -role=compactor` for the compaction stage. The default `-role=all` preserves the original behavior: claim `READY` source work first, then compact when no `READY` work is available. `-compact=false` is only valid for `-role=all` or `-role=inserter`; it disables opportunistic compaction for `all` workers and disables the 5 minute source merge wait cap.
...
Workers run insert-select queries with an insert memory cap of 70% of detected memory. Initial insert concurrency is the lower of about one quarter of the detected CPU count and a memory-derived limit that targets at least 2 GiB insert blocks when memory allows, so CPU-rich workers with limited memory start less aggressively while high-memory workers can still use more than eight insert threads. The worker derives `min_insert_block_size_bytes` from the insert memory cap divided by six times the insert thread count, then derives `min_insert_block_size_rows` from that byte target using a 1 KiB average-row estimate. This gives larger insert blocks while leaving about half of the query memory cap for read, decompression, and expression overhead. Before the insert starts, the worker applies `default_compression_codec` to the worker destination table; the default is `ZSTD(5)` and can be changed with `-default-compression-codec`. After the insert completes, the worker applies memory-derived `merge_max_block_size` and `merge_max_block_size_bytes`, a lower local `merge_selecting_sleep_ms`, and 150 GiB for both `max_bytes_to_merge_at_max_space_in_pool` and `max_bytes_to_merge_at_min_space_in_pool` to the worker destination table. This keeps local workers merging toward roughly 150 GiB physical parts whenever a pair of active parts can merge under that target. The worker then restarts its local ClickHouse child process with `background_pool_size` set to the larger of the detected CPU count and ClickHouse's minimum valid pool size for default MergeTree mutation settings and `background_merges_mutations_scheduling_policy = 'shortest_task_first'`. It then lets destination merges run until they settle or `-merge-max-runtime` is reached. When compaction is enabled, this initial source rewrite merge wait is capped at 5 minutes; deeper merge waiting is handled by the compaction path. If no merge is active but multiple active output parts remain, the worker keeps polling until no partition has any pair of active parts that can merge under the 150 GiB target, or until the hard merge max runtime is reached.

Compaction workers heartbeat claimed `COMPACTING` rows and automatically release stale compaction claims after the derived lease timeout, based on `-compact-window` with a 5 minute floor. The compact window is also the hard cap for a claimed compact batch when it is greater than zero. `-compact-max-artifacts` limits the number of input state rows claimed into one compaction batch; it defaults to 8 and can be raised up to 99 for many small artifacts, but disk safety should be controlled with `-compact-max-bytes`. Keep that byte cap well below free disk because compaction stores downloaded tarballs, extracted or attached parts, ClickHouse merge output, and uploaded tarballs during different phases. When the compact deadline is reached, the worker measures the current local output: if the active part count was reduced, it uploads the current compact artifact; otherwise it releases the inputs and finalizes them when the job is eligible. Finalization uses the newest current `COMPACT_READY` or `COMPACTING` timestamp, and successful compact outputs inherit the newest input compact-ready timestamp, so deeper compaction does not extend the job-level window automatically.
...
Use `-json` on either command for machine-readable output. `job-status` includes physical ClickHouse part counters: `input_clickhouse_parts` is the original source part count, and `current_output_clickhouse_parts` is the active output part count represented by non-superseded in-flight or finished rows. During compaction, it also reports compact-ready/compacting counts and when compact-ready parts can finalize. Use `job-status -parts` to include one row per PartForge state row with latest persisted rewrite counters, compact-ready age, destination partitions, active physical part stats, and `FAILED_MERGES`, or `job-status -details` to include each part's current rewrite stage and per-stage timings. `list-jobs` queries the status index, so admin IAM needs `dynamodb:Query` on the table's `gsi1` index.
...
Part state is stored in DynamoDB. Inserter workers claim work with conditional updates from `READY` to `IN_PROGRESS`; handled processing errors are written as `FAILED`; successful rewrites become `COMPACT_READY`; compactor workers claim `COMPACT_READY` rows as `COMPACTING` and produce smaller `COMPACT_READY` rows or finalize rows as `FINISHED`; and `import-finished` transitions finished parts through `IMPORTING` to `IMPORTED`. On SIGINT or SIGTERM, a worker stops claiming new work immediately. Active insert work is canceled and returned to `READY`. Active compaction stops waiting for more merge progress, then saves a compact artifact only if the current output has fewer physical ClickHouse parts than the input; otherwise it releases the batch back to `COMPACT_READY`. If a worker process dies outside handled code, the part remains visible as `IN_PROGRESS` or `COMPACTING` for manual inspection or reset.
