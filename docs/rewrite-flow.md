...
The compactor downloads and attaches the claimed artifact before starting the merge wait. ClickHouse assigns attached part names, so the worker does not rename parts before attach. The worker stops background merges before attach, starts them afterward, runs partition-scoped `OPTIMIZE FINAL` queries one at a time, and then verifies through `system.merges` and `system.parts` that no merge remains and every original destination partition has exactly one active part. It uploads nothing unless that verification succeeds.
