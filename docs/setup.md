...
You don't need to hand-tune performance. The worker derives the insert memory cap and `max_threads` / `max_insert_threads` from the container's detected CPU and memory, keeps the default block size on the first attempt and halves it on resource-error retries down to 8,192 rows, caps JSON String buffer growth increments at 64 MiB, and derives merge settings. Rewrites use the destination schema's compression settings without an override; `-default-compression-codec` (default `ZSTD(5)`) applies only during compaction. A query-level `SETTINGS` clause in your `INSERT ... SELECT` is passed through to ClickHouse if you have a specific need, but avoid overriding the memory/thread settings the worker manages. See [operations.md](operations.md) and [rewrite-flow.md](rewrite-flow.md).
